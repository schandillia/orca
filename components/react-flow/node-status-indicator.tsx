...
export const SpinnerLoadingIndicator = ({
  ...
  return (
    <div className="relative">
      ...
      <div className="bg-background/50 backdrop-blur-xs absolute inset-0 z-50 rounded-sm" />
      <div className="absolute inset-0 z-50">
        ...
        <IconLoader className="absolute left-[calc(50%-0.75rem)] top-[calc(50%-0.75rem)] size-6 animate-spin text-blue-700" />
      </div>
    </div>
  )
}
...
const StatusBorder = ({
  ...
  return (
    <>
      <div
        className={cn(
          "absolute -left-px -top-px h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-sm border-2",
          className,
        )}
      />
      {children}
